...
import { BankerDrawer } from "./CategoryDrawer";
import { columns, exportColumns } from "./columns";
import { Banker, emptyBanker } from "./data";

export default function BankerPage() {
  const [data, setData] = useState<Banker[]>(() =>
    masterStorage.getBankers(),
  );
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      if (
        filterName &&
        !item.bankerName.toLowerCase().includes(filterName.toLowerCase())
      )
        return false;
  ...
  const persist = (next: Banker[]) => {
    setData(next);
    masterStorage.saveBankers(next);
  };
  ...
  return (
    <Page title="Banker">
      <div className="transition-content w-full pb-5">
        <MasterToolbar
          title="Banker"
          createLabel="Create Banker"
          searchPlaceholder="Search bankers..."
          table={table}
          showFilters={showFilters}
          onToggleFilters={() => setShowFilters((v) => !v)}
          onCreate={() => {
            setEditing(emptyBanker());
            setDrawerOpen(true);
          }}
          onExportExcel={() =>
            exportToExcel(filteredData, exportColumns, "bankers")
          }
          onExportPdf={() =>
            exportToPdf(
              filteredData,
        ...
        <MasterTable
          ...
          emptyMessage="No bankers found. Click Create Banker to add one."
        />
      </div>

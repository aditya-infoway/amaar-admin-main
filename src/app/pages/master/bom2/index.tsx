...
export default function BOM2Page() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const response = await Get("master/bom2/list", {}, false);
      if (response.data?.success) {
        setData((response.data.data || []).map(mapApiBOM2ToBOM2));
      } else {
        toasterrormsg(response.data?.message || "Failed to fetch BOM2 items.");
      }
    } catch (error) {
  ...
  const table = useReactTable({
    ...
    getRowId: (row) => row.id,
    meta: {
      openEditDrawer: (row: BOM2) => navigate(`/master/bom2/edit/${row.id}`),
      deleteRow: (row) => {
    ...
    <Page title="BOM">
      <div className="transition-content w-full pb-5">
        <MasterToolbar
          ...
          onCreate={() => navigate("/master/bom2/create")}
          onExportExcel={() =>
        ...
        <MasterTable
          ...
          emptyMessage={
            loading
              ? "Loading BOM2 items..."
              : "No BOM2 items found. Click Create BOM2 to add one."
          }
        />
      </div>
    </Page>

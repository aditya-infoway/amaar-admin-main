...
export function PersonalInfo({ setCurrentStep }: PersonalInfoProps) {
  ...
  const { register, control, handleSubmit, watch, setValue } = useForm<TyreBOMFormType>({
      ...
      bom: [
        {
          ...
          cost: 0,
          children: [
            { name: "Ring", type: "item", quantity: 1, cost: 250 },
            { name: "Bolt", type: "item", quantity: 10, cost: 4000 },
          ],
        },
        {
          name: "Tube",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} onKeyDown={handleKeyDown} autoComplete="off" className="w-full mx-auto px-0 sm:px-4">
                  ...
                  <div className="flex items-start gap-2 flex-1 w-full">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 flex-1 w-full max-w-md">
                      <Input
                        ...
                        placeholder={isAssembly ? "Assembly Name (e.g. Wheel Ring)" : "Item Name (e.g. Tube)"}
      ...
      <div className="mt-5 flex justify-end space-x-2 border-t border-gray-100 pt-3 dark:border-dark-500">
        <Button variant="outlined" className="px-4 h-8 text-xs border border-gray-300 dark:border-dark-600">Cancel</Button>
